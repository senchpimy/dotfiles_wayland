// CC0: Hard light grid tracer
//  Inspired by this tweet: https://twitter.com/lainmell/status/1629414269019357184?s=20
//  Fixed for standalone by replacing iChannel1 with procedural noise.

#ifdef GL_ES
precision highp float;
#endif

// Variables uniform estándar (descomenta si tu entorno las necesita explícitas)
...
// --- FUNCIÓN DE RUIDO AÑADIDA PARA REEMPLAZAR LA TEXTURA ---
// Genera un valor aleatorio entre 0.0 y 1.0 basado en la posición p
float hash21(vec2 p) {
...
vec3 baseCol(vec2 p) {
  // Originalmente usaba texture(iChannel1, p * 0.00125).x
  // Usamos hash21 directamente sobre las coordenadas enteras
  float h = hash21(p); 
  h -= 0.5;
  h *= 2.0;
...
float height(vec2 p) {
  ...
  // Reemplazo de textura por ruido procedural
  // Original: float h = texture(iChannel1, p * 0.001).x;
  float h = hash21(p);
  
  // Simulamos la variación que tenía la textura original
  h *= h;
...
float cellTrace(
  ...
  // Corregido: inicializar 'i' a 0 explícitamente
