/*
 * Original shader logic with cleanup for C++ Viewer
 */

// Emulate a black texture (Keep these to prevent errors since the viewer doesn't load textures)
...
float Body(vec3 point)
{
    ...
    dist = smin(dist, sdCapsule(original+vec3(0.0,0.3,-0.7), vec3(0.0, 0.0, 0.0), vec3(0.0, 0.0+sin(iTime*2.2)*0.15, 2.5), .15), 7.0);
...
vec3 GetBaseColor(vec3 pos, int material)
{
    ...
    {
        // Notice: iChannel1 is disabled by macro, returns 0.0, effectively disabling texture lookup
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // Creates black bars at the top and bottom
    // if(abs(uv.y) > .75)
    //     return;    
