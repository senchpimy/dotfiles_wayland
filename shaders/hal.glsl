...
// Descomenta estas líneas si tu editor no las provee automáticamente:
// uniform vec3 iResolution;
// uniform float iTime;
// uniform sampler2D iChannel0; 

// --- FUNCIONES SDF RENOMBRADAS PARA EVITAR ERRORES DE SINTAXIS ---

// Renombrado de 'box' a 'sdBox2D' para evitar conflictos
...
// Renombrado de 'line' a 'sdLine' porque 'line' es palabra reservada
float sdLine( vec2 p, vec3 d ) 
{
...
{
    // A
    float d = sdBox2D(p-vec2(0.,0.),vec2(.74,.48)*.4); // Mask
    if (d<0.1) {
    	float c = sdLine(p-vec2(-0.12,0.),vec3(-0.48,.26,.05)); // Left edge
    	c = min(c,sdLine(p-vec2(0.12,0.),vec3(.48,.26,.05))); // Right edge
    	c = min(c,sdBox2D(p-vec2(0.,-0.08),vec2(0.19,0.04))); // Bar
        d = max(c,d);
    }
    return d;
}
...
{
    // B
    float d = sdBox2D(p-vec2(0.,0.),vec2(.63,.48)*.4); // Mask
    if (d<0.1) {
	    float c = sdLine(p-vec2(-.06,0.),vec3(1.,0.,0.5*.4)); // Left edge
        ...
        c = max(c,-sdBox2D(p-vec2(-0.025,0.08),vec2(0.11,0.035))); // Bar
    	c = max(c,-sdBox2D(p-vec2(-0.02,-0.07),vec2(0.115,0.035))); // Bar
        d = max(c,d);
    }
    return d;
}
...
{
    // D
    float d = sdBox2D(p-vec2(0.,0.),vec2(.65,.48)*.4); // Mask
    if (d<0.1) {
	    float c = squircle(p/vec2(.65,.48),vec3(.4,2.3,-.5)); // Outer
    	c = min(c,sdBox2D(p-vec2(-.15,0.),vec2(.55,.48)*.4)); 
	    c = max(c,-squircle(1.76*p/vec2(.65,.48),vec3(.4,2.9,-.6))); // Inner
    	c = max(c,-sdBox2D(p-vec2(-.03,0.),vec2(.55,.48)*.225));
        d = max(c,d);
    }
    return d;
}
...
{
    // E
    float d = sdBox2D(p-vec2(0.,0.),vec2(.54,.48)*.4); // Mask
    if (d<0.1) {
	    float c = sdBox2D(p-vec2(-0.16,0.),vec2(.14,.48)*.4);
	    c = min(c,sdBox2D(p-vec2(0.1,0.16),vec2(.54,.13)*.4)); 
 	    c = min(c,sdBox2D(p-vec2(0.0,0.0),vec2(.47,.095)*.4)); 
 	    c = min(c,sdBox2D(p-vec2(0.1,-0.16),vec2(.54,.13)*.4));
        d = max(c,d);
    }
    return d;
...
float esG( vec2 p ) 
{
    ...
    if (d<0.1) {
	    float c = squircle(p/vec2(.61,.48),vec3(.4,2.3,-.5)); // Outer
	    c = max(c,-squircle(1.8*p/vec2(.61,.48),vec3(.4,2.9,-.6))); // Inner
    	c = max(c,-sdBox2D(p-vec2(0.2,0.02),vec2(0.11,0.05))); // Hole
 	    c = min(c,sdBox2D(p-vec2(0.13,-0.01),vec2(.33,.08)*.4)); 
        d = max(c,d);
    }
    return d;
}
...
float esH( vec2 p ) 
{
    ...
    float d = sdBox2D(p-vec2(0.,0.),vec2(.63,.48)*.4); // Mask
    if (d<0.1) {
	    float c = sdBox2D(p-vec2(-0.195,0.),vec2(.14,.48)*.4);
 	    c = min(c,sdBox2D(p-vec2(0.0,0.0),vec2(.47,.095)*.4)); 
	    c = min(c,sdBox2D(p-vec2(0.196,0.),vec2(.14,.48)*.4));
        d = max(c,d);
    }
    return d;
}
...
{
    // I
    float d = sdBox2D(p-vec2(0.,0.),vec2(.16,.48)*.4); // Mask
    if (d<0.1) {
	    float c = sdBox2D(p-vec2(0.,0.),vec2(.14,.48)*.4);
        d = max(c,d);
    }
    return d;
}
...
{
    // M
    float d = sdBox2D(p-vec2(0.,0.),vec2(.83,.48)*.4); // Mask
    if (d<0.1) {
	    float c = sdBox2D(p-vec2(-0.28,0.),vec2(.14,.48)*.4);
	    c = min(c,sdBox2D(p-vec2(0.28,0.),vec2(.14,.48)*.4));
    	c = min(c,sdLine(p-vec2(-0.11,0.),vec3(0.48,.26,.05))); // Left edge
    	c = min(c,sdLine(p-vec2(0.11,0.),vec3(-.48,.26,.05))); // Right edge
        d = max(c,d);
    }
    return d;
}
...
{
    // N
    float d = sdBox2D(p-vec2(0.,0.),vec2(.68,.48)*.4); // Mask
    if (d<0.1) {
	    float c = sdBox2D(p-vec2(-0.22,0.),vec2(.14,.48)*.4);
	    c = min(c,sdBox2D(p-vec2(0.22,0.),vec2(.14,.48)*.4));
    	c = min(c,sdLine(p-vec2(-.0,0.),vec3(0.3,.26,.05))); // Left edge
        d = max(c,d);
    }
    return d;
}
...
{
    // T
    float d = sdBox2D(p-vec2(0.,0.),vec2(.58,.48)*.4); // Mask
    if (d<0.1) {
	    float c = sdBox2D(p-vec2(0.,0.15),vec2(.54,.12)*.4); 
	    c = min(c,sdBox2D(p-vec2(0.0,0.),vec2(.14,.48)*.4));
        d = max(c,d);
    }
    return d;
}
...
{
    // V
    float d = sdBox2D(p-vec2(0.,0.),vec2(.73,.48)*.4); // Mask
    if (d<0.1) {
    	float c = sdLine(p-vec2(-0.12,0.),vec3(.48,.26,.06)); // Left edge
    	c = min(c,sdLine(p-vec2(0.12,0.),vec3(-.48,.26,.06))); // Right edge
        d = max(c,d);
    }
    return d;
}
...
{
    // 1
    float d = sdBox2D(p-vec2(0.,0.),vec2(.38,.48)*.4); // Mask
    if (d<0.1) {
	    float c = sdBox2D(p-vec2(0.1,0.),vec2(.14,.48)*.4);
    	c = min(c,sdLine(p-vec2(-.0,0.15),vec3(-0.15,.22,.04))); // Left edge
    	c = max(c,-sdLine(p-vec2(-.3,0.13),vec3(0.22,.15,.1))); // Left edge
        d = max(c,d);
    }
    return d;
}
...
float es3( vec2 p ) 
{
    ...
    if (d<0.1) {
        ...
        c = min(e,c);
        c = max(c,-sdBox2D(p-vec2(-0.15,-0.0),vec2(.35,.18)*.4));
        d = max(c,d);
    }
    return d;
}
...
float es4( vec2 p ) 
{
    ...
    float d = sdBox2D(p-vec2(0.,0.),vec2(.61,.48)*.4); // Mask
    if (d<0.1) {
	    float c = sdBox2D(p-vec2(0.11,0.),vec2(.14,.48)*.4);
 	    c = min(c,sdBox2D(p-vec2(0.0,-0.06),vec2(.61,.095)*.4)); 
    	c = min(c,sdLine(p-vec2(-.01,0.13),vec3(-0.18,.26,.045))); // Left edge
        d = max(c,d);
    }
    return d;
}
...
float es5( vec2 p ) 
{
    ...
    float d = sdBox2D(p-vec2(0.,0.),vec2(.58,.48)*.4); // Mask
    if (d<0.1) {
	    float c = sdBox2D(p-vec2(-0.16,0.1),vec2(.14,.37)*.4);
	    c = min(c,sdBox2D(p-vec2(0.02,0.16),vec2(.48,.13)*.4));         
	    float e = squircle(p/vec2(.57,.27)-vec2(0.,-.33),vec3(.4,2.2,-.5)); // Outer
	    e = max(e,-squircle(1.9*p/vec2(.57,.22)-vec2(0.,-.71),vec3(.4,2.,-.6))); // Inner
	    e = max(e,-sdBox2D(p-vec2(-0.21,-0.05),vec2(.26,.15)*.4));         
        c = min(e,c);
        d = max(c,d);
    }
    return d;
...
float es6( vec2 p ) 
{
    ...
    if (d<0.1) {
	    ...
	    c = max(c,-squircle(1.8*p/vec2(.57,.22)-vec2(0.,0.65),vec3(.4,2.,-.6))); // Inner
        c = max(c,-sdBox2D(p-vec2(0.,.02),vec2(.65,.18)*.4));
	    float e = squircle(p/vec2(.57,.27)-vec2(0.,-.33),vec3(.4,1.5,-.5)); // Outer
	    e = max(e,-squircle(1.8*p/vec2(.57,.22)-vec2(0.,-.65),vec3(.4,2.,-.6))); // Inner
        c = min(e,c);
	    c = min(c, sdBox2D(p-vec2(-0.183,0.),vec2(.14,.25)*.4));
        d = max(c,d);
    }
    return d;
}
...
float es7( vec2 p ) 
{
    ...
    float d = sdBox2D(p-vec2(0.,0.),vec2(.58,.48)*.4); // Mask
    if (d<0.1) {
	    float c = sdBox2D(p-vec2(.0,.15),vec2(.54,.11)*.4); 
    	c = min(c,sdLine(p-vec2(.085,0.),vec3(-0.37,.32,.04))); // Left edge
        d = max(c,d);
    }
    return d;
}
...
float es9( vec2 p ) 
{
    ...
    if (d<0.1) {
	    ...
	    float e = squircle(p/vec2(.57,.27)-vec2(0.,-.33),vec3(.4,1.5,-.5)); // Outer
	    e = max(e,-squircle(1.8*p/vec2(.57,.22)-vec2(0.,-.65),vec3(.4,2.,-.6))); // Inner
        e = max(e,-sdBox2D(p-vec2(0.,-.03),vec2(.6,.18)*.4));
        c = min(e,c);
	    c = min(c, sdBox2D(p-vec2(0.1775,0.0),vec2(.125,.255)*.4));
        d = max(c,d);
    }
    return d;
}
...
{
    // I
    float d = sdBox2D(p-vec2(0.,0.),vec2(.14,.48)*.4); // Mask
    if (d<0.1) {
	    float c = sdBox2D(p-vec2(0.,-0.14),vec2(.14,.14)*.4);
	    c = min(c, sdBox2D(p-vec2(0.,0.05),vec2(.14,.14)*.4));
        d = max(c,d);
    }
    return d;
}
...
{
    // I
    float d = sdBox2D(p-vec2(0.,0.),vec2(.34,.48)*.4); // Mask
    if (d<0.1) {
	    float c = sdBox2D(p-vec2(0.,0.-.03),vec2(.34,.14)*.4);
        d = max(c,d);
    }
    return d;
}
...
float cx(int ix, vec3 lab) {
   if (ix==0) return lab.x;
   else if (ix==1) return lab.y;
   else return lab.z;
}

float six(int ix, float sp) {
    if (ix==0) return ran(sp,277.,313.,14.); 
    else if (ix==1) return ran(sp,173.,311.,14.);
    else if (ix==2) return ran(sp,113.,433.,14.); 
    else if (ix==2) return 24.;// Colon
    else if (ix==2) return ran(sp,157.,421.,10.)+14.;
    else if (ix==2) return ran(sp,119.,133.,10.)+14.;
    else if (ix==2) return 25.;// Dash
    else if (ix==2) return ran(sp,139.,313.,14.);
    else return ran(sp,119.,137.,14.);
}
...
vec3 bigtext( vec2 p, float sp, float screenNum, float pixwid ) 
{
    ...
    float w[9];for (int i=0;i<9;i++) w[i] = chW(six(i,sp))*sw;
    ...
    if (p.x>-.98 && p.x<.98) {
        if ((p.y<-0.06)&&(p.y>-.35)) {
            ...
            int ix=-1;
            float mx;
            for (int i=8;i>=0;i--) { if ((p.x+ma.x)<(x[i]+w[i])) { ix=i; mx = ma.x-x[i]; } }
            sc = sc2;
            ma.x = p.x+mx;
            if (ix<=8 && ix>=0) cix = six(ix,sp);
...
vec4 screen( vec2 p, float spf, float screenNum, float aa )
{
    ...
    float flicker = (1.0+0.04*sin((iTime+screenNum)*1000.)); // Back-projection flicker
    // Colours
...
float halfont( vec2 p )
{
    ...
    p = vec2(xx,p.y);
    float r = 1.0;
    float outer = sdBox2D(p,vec2(7.,11.)*sc);
    if (cn==-3.) {
        float htop = sdBox2D(p-vec2(0.,9.25)*sc,vec2(1.7,5.75)*sc);
        float hbot = sdBox2D(p+vec2(0.,6.25)*sc,vec2(1.7,5.75)*sc);
        outer = max(outer,-htop);
        outer = max(outer,-hbot);
        r = abs(outer+.5*sc)-.01*sc;
    } else if (cn==-2.) {
        float left = sdLine(p-vec2(-2.4,0.)*sc,vec3(-1.,.2,2.3*sc));
        float right = sdLine(p-vec2(2.4,0.)*sc,vec3(1.,.2,2.3*sc));
        float bar = sdBox2D(p+vec2(0.,5.)*sc,vec2(5.,2.25)*sc);
        left = min(left,right);
        left = min(left,bar);
        outer = max(left,outer);
        r = abs(outer+.5*sc)-.01*sc;
    } else if (cn==-1.) {
        float htop = sdBox2D(p-vec2(4.5,4.)*sc,vec2(7.,11.)*sc);
        float shrink = sdBox2D(p-vec2(7.,-9.)*sc,vec2(2.,4.)*sc);
        outer = max(outer,-htop);
        ...
        float cut = sdBox2D(p-vec2(-4.,-1.)*sc,vec2(4.,3.)*sc);
...
vec4 hal( vec2 p, float aa, float mode ) 
{
    float ref = 0.0;
    float sc = 0.0033;
    float outer = abs(sdBox2D(p-vec2(.0,-1.1),vec2(114.3,349.25)*sc)+3.175*.5*sc)-3.175*.5*sc;
    float lowerline = sdBox2D(p-vec2(.0,-1.1-200.025*sc),vec2(114.,3.175*.5)*sc);
    float woodbox = sdBox2D(p-vec2(.0,-.86),vec2(114.4,270.0)*sc);
    ...
    float logo = sdBox2D(p-vec2(.0,-1.1+310.*sc),vec2(92.,18.)*sc);
    float logo2 = sdBox2D(p-vec2(.0+48.5*sc,-1.1+310.*sc),vec2(44.75,18.75)*sc);
    float logofont = 1.0; 
    if (logo<0.) {
    ...
    float speaker = sdBox2D(p-vec2(.0,-1.1-273.5*sc),vec2(109.,69.)*sc);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    if (m.y==1.) {
        ...
        if (fuv.x!=0. && fuv.x<3. && fuv.x>-3. && fuv.y<=0. && fuv.y>=-1.) {
            ...
            alb = screen(muv,(iTime*.05+sn*93.16),sn,aa);
        } else {
    ...
    if (d<1e-1) {
        ...
        // --- REFLEJOS PROCEDURALES (Sin textura) ---
        vec3 refcol = vec3(pow(max(0.0, dot(ref, lightDir)), 20.0)); 
        vec3 refcol2 = vec3(0.25);
        // -------------------------------------------
