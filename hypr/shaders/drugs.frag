...
vec3 rgb(in vec2 tc, float freq, float amp, inout vec4 centre) {
    vec2 off = vec2(1.0/800.0, 0.0) * sin(tc.y * freq + time) * amp;
    vec2 off2 = vec2(1.0/800.0, 0.0) * sin(tc.y * freq - time * 1.5) * amp;
...
void main() {
    ...
    float mix_factor = (sin(time) + 1.0) * 0.5;
    vec3 col = mix(centre.rgb, bent, mix_factor);
