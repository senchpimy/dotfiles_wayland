...
AbstractWidget {
    ...
    onReleased: {
        ...
        // Guardar usando la función helper para asegurar la persistencia
        const basePath = "background.widgets." + configEntryName;
        Config.setNestedValue(basePath + ".x", Math.round(root.x));
        Config.setNestedValue(basePath + ".y", Math.round(root.y));
        
        // Si no era "free", cambiarlo a "free" para que mantenga la posición
        if (root.placementStrategy !== "free") {
             Config.setNestedValue(basePath + ".placementStrategy", "free");
        }
    }
